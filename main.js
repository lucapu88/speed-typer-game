...
const inputText = document.getElementById('input-word-rewrite'); // vabbè si sa
...
function updateTime() {
  ...
  if (countdown === 0) {
    ...
    finalTotalScore = initialScore;
    saveScoreAndTime(finalTotalScore, timer, difficultyName);
    gameOver();
  }
}
...
const source = document.querySelector('.h2-word-rewrite');
...
inputText.addEventListener('keyup', compareWrittenWords);
inputText.addEventListener('touchend', compareWrittenWords);

function compareWrittenWords(e) {
  if (e.isTrusted) {
...
function onGameOverChangeHtml() {
  ...
  totalScoreText.innerHTML = `Your score: <span id="final-score">${finalTotalScore}</span>`;
...
function controls() {
  ...
  if (+finalScoreHtml.innerText !== finalTotalScore) {
    vaBene = false;
  }
}
